...
const gqlReq = (tableName: string) => {
     ...
     * ```
     */
    const find = (condition: ICondition, fields: string, axiosConfig?: AxiosRequestConfig) => {
        const getGqlData = (condition: ICondition) =>
            `{find_${tableName}(${condition ? obj2gqlString(condition) : 'where:{}'})${fields || {}}}`;
     ...
     * ```
     */
    const findAll = async (condition: ICondition, fields = '{}', axiosConfig?: AxiosRequestConfig) => {
        if (condition.limit) {
